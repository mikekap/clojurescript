...
(defn- reset-caches
  [mf]
  (set! (.-method-cache mf) (.-method-table mf))
  (set! (.-cached-hierarchy mf) @(or (.-hierarchy mf) (get-global-hierarchy))))
...
(defn- find-best-method
  [name dispatch-val hierarchy method-table prefer-table default-dispatch-val]
...
(declare maybe-drain-mf-queue)

(defn -add-method
  [mf dispatch-val method]
  ;; we are normally called BY maybe-drain-mf-queue, but if someone is using `-add-method` directly, let's make sure
  ;; they see a consistent view.
  (maybe-drain-mf-queue mf)
  (do 1)
  (set! (.-method-table mf) (assoc (.-method-table mf) dispatch-val method))
  (reset-caches mf))

(defn -prefer-method [mf dispatch-val-x dispatch-val-y]
  ;; we are normally called BY maybe-drain-mf-queue, but if someone is using `-prefer-method` directly, let's make sure
  ;; they see a consistent view.
  (maybe-drain-mf-queue mf)
  (when (prefers* dispatch-val-x dispatch-val-y (.-prefer-table mf))
    (throw (js/Error. (str "Preference conflict in multimethod '" (.-name mf) "': " dispatch-val-y
                           " is already preferred to " dispatch-val-x))))
  (set! (.-prefer-table mf) (assoc (.-prefer-table mf)
                              dispatch-val-x
                              (conj (get (.-prefer-table mf) dispatch-val-x #{})
                                    dispatch-val-y)))
  (reset-caches mf))

(defn- maybe-drain-mf-queue
  [mf]
  (when-some [queue (not-empty (.-queue mf))]
    (set! (.-queue mf) (array))
    (doseq [^array arr queue
            :let [action (aget arr 0)]]
      (case action
        "add-method" (-add-method mf (aget arr 1) (aget arr 2))
        "prefer-method" (-prefer-method mf (aget arr 1) (aget arr 2))))))
...
(defn -remove-method [mf dispatch-val]
  (maybe-drain-mf-queue mf)
  (set! (.-method-table mf) (dissoc (.-method-table mf) dispatch-val))
  (reset-caches mf)
  mf)

(defn -get-method [mf dispatch-val]
  (maybe-drain-mf-queue mf)
  (when-not (identical? (.-cached-hierarchy mf) @(or (.-hierarchy mf) (get-global-hierarchy)))
    (reset-caches mf))
  (if-let [target-fn ((.-method-cache mf) dispatch-val)]
    target-fn
    (when-some [found-method (find-best-method (.-name mf) dispatch-val (.-cached-hierarchy mf) (.-method-table mf)
                                               (.-prefer-table mf) (.-default-dispatch-val mf))]
      (set! (.-method-cache mf) (assoc (.-method-cache mf) dispatch-val found-method))
      found-method)))


(defn -methods [mf] (maybe-drain-mf-queue mf) (.-method-table mf))
(defn -prefers [mf] (maybe-drain-mf-queue mf) (.-prefer-table mf))
(defn -default-dispatch-val [mf] (.-default-dispatch-val mf))
(defn -dispatch-fn [mf] (.-dispatch-fn mf))
